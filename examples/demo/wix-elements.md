...
> Source: `.wix/types/*` (gitignored, refreshed by `wix sync-types`). Generated: 2026-06-11T05:56:22.172Z
...
- A **`Box`/`MultiStateBox` has NO `.text`.** Calling `.text` on a container is the classic bug — set its **child** Text elements, or use `.changeState()` for a MultiStateBox.

## 🌐 Global elements (header / footer / menus — addressable on EVERY page)

| ID | Type | State | Layout (x,y · w×h) | How to set it (Velo) |
|----|------|-------|--------------------|----------------------|
